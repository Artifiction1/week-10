...
function App() {
  let [City_List, setCity_List] = useState(null)
  const CityList = () => {
    let city_list = []
    ...
    for (let i = 0; i < 15; i++) {
      ...
      city_list[i] = city_names[x]
    }
    return city_list
  }
  useEffect(()=>{
    setCity_List(CityList())
},[])
useEffect(()=>{
})
if(City_List !== null){
    ...
    <div>
      ...
      <Container>
        <Routes>
          <Route path='/' element={
        <Row>
          {City_List.map(city => { return <Weather city={city} /> })}
        </Row>}/>
        <Route path='/Dayly/' element={<Dayly/>}/>
